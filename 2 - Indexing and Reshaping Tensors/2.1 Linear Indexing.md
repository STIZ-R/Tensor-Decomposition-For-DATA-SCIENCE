...
> Let $\matbb{A} \in \mathbb{R}^{m \times q}$, $\matbb{B} \in \mathbb{R}^{n \times r}$, $\mathbb{C} \in \mathbb{R}^{p \times s}$, and define their *reverse* Kronecker Product $V = C \otimes B \otimes A$.
