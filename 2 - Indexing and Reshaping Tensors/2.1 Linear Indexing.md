...
> The **strides** for the **reverse ordering** are  
> $ s_d^{*} = 1 \text{ and } s_{k-1}^{*} = \prod_{l=k}^d n_l = s_k^{*} n_k \text{ for } k = d, \dots, 2. $  
>
> The **(reverse) linear index** of  
> $ (i_1, i_2, \dots, i_d) \in [n_1] \otimes [n_2] \otimes \cdots \otimes [n_d] $ is  
> $ \alpha = \mathbb{L}^{*}(i_1, i_2, \dots, i_d)
>     = 1 + \sum_{k=1}^d s_k^{*} (i_k - 1). $  
>
> The **(reverse) tuple index** of $ \alpha \in [N] $ with  
> $ N = \prod_{k=1}^d n_k $ is  
> $ (i_1, i_2, \dots, i_d) = \mathbb{T}^{*}(\alpha), $ where  
> $ i_k = \mathbb{T}_k^{*}(\alpha)
>     = 1 + \left\lfloor
>         \dfrac{(\alpha - 1) \bmod (n_k s_k^{*})}{s_k^{*}}
>       \right\rfloor. $
