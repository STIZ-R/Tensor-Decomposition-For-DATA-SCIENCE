...
> The **strides** for the **general ordering** are $\bar{s}_{\pi_1} = 1$ and $\bar{s}_{\pi_{k+1}} = \prod_{l=1}^kn_{\pi_l} = \bar{s}_{\pi_k}n_{\pi_k}$ for $k \in [d - 1]$.  
> The **(general) linear index** of $(i_1, i_2, \dots , i_d) \in [n_1] \otimes [n_2] \otimes \dots \otimes [n_d]$ is $\alpha = \mathbb{L}^{(\pi)}(i_1,i_2, \dots , i_d) = 1 + \sum_{k=1}^d\bar{s}_k(i_k - 1)$.  
> The **(general) tuple index** of $\alpha \in [N]$ with $N = \prod_{k=1}^dn_k$ is $(i_1,i_2, \dots ,i_d) = \mathbb{T}^{(\pi)}(\alpha)$, where $i_k = \mathbb{T}_k^{\pi}(\alpha) = 1 + \left\lfloor\dfrac{(\alpha - 1) \bmod (n_k\bar{s}_k)}{\bar{s}_k}\right\rfloor$.
>
