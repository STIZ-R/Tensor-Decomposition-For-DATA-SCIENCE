...
\textbf{Definition (Mode-$k$ Unfolding).}
The \textbf{mode-$k$ unfolding} of a tensor $\mathbf{x}$ of size
$n_1 \times n_2 \times \dots \times n_d$ is the matrix
$\mathbf{X}_{(k)} \in \mathbb{R}^{n_k \times N_k}$,
where
\[
...
and
\[
\mathbf{X}_{(k)}(i_k, \beta_k) = \mathbf{x}(i_1, \dots, i_d),
\]
with
\[
\beta_k = \mathcal{L}(i_1, \dots, i_{k-1}, i_{k+1}, \dots, i_d).
\]
