...
## 2.3.3 Structure of Mode-*k* Unfoldings

This section illustrates how tensor unfoldings determine the resulting matrix dimensions.  
As an example, consider a 4-way tensor of size $3 \times 4 \times 3 \times 2$.

![Figure 4](../image/Figure1.png)


The mode-1 unfolding is naturally aligned with column-major storage: $\text{vec}(X_{(1)}) = \text{vec}(X)$ and requires no data movement.

For $k \neq 1$, mode-$k$ unfoldings are not globally column-major, but they have a **block structure**.  
Define:
```math
M_k = \prod_{\ell=1}^{k-1} n_\ell, \quad
P_k = \prod_{\ell=k+1}^{d} n_\ell
```

The unfolding $X_{(k)}$ consists of $P_k$ blocks of size $n_k \times M_k$, each stored contiguously in **row-major order**.

Special cases:
```math
\text{vec}(X_{(1)}) = \text{vec}(X), \quad
\text{vec}(X_{(d)}^T) = \text{vec}(X)
```

This block structure enables efficient matrix operations (e.g., Gram matrices) **without explicit tensor reordering**.
