...
> The **mode-*k* unfolding** of a tensor $\mathbf{x}$ of size $n_1 \times n_2 \times \dots \times n_d$ is the matrix $\mathbf{X}_{(k)} \in \mathbb{R}^{n_k \times N_k},$ where $N_k = \prod_{\substack{l=1 \\ l \neq k}}^d n_l$ and $\mathbb{X}_{(k)}(i_k, \beta _k) = \mathbf{x}(i_1 , \dots, i_d)$ with \beta _k = \mathbb{L}9i_1, \dots, i_{k-1}, i_{k+1}, \dots, i_d).$
