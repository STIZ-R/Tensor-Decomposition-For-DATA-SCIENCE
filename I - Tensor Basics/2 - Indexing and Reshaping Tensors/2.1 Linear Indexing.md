# 2 - Indexing and Reshaping Tenssors
# 2.1 Linear Indexing  
...
We now clarify the meaning of the symbol $\otimes$, which denotes the **Kronecker product**.
Intuitively, the Kronecker product consists in replicating the local structure of one matrix according to the global structure of another.
...
We also briefly introduce the **Khatri–Rao product**, which will be useful later.

Using the same matrices, $A$ and $B$, let $a_1, a_2$ and $b_1, b_2$ denote their respective columns:
```math
a_1 = \begin{bmatrix} 1 \\ 3 \end{bmatrix}, \quad a_2 = \begin{bmatrix} 2 \\ 4 \end{bmatrix}, \quad b_1 = \begin{bmatrix} 0 \\ 6 \end{bmatrix}, \quad b_2 = \begin{bmatrix} 5 \\ 7 \end{bmatrix}, \quad
```
The Khatri–Rao product is defined columnwise as:
```math
A \odot B = (a_1 \otimes b_1  a_2 \otimes b_2) = \begin{bmatrix} 0 & 10 \\ 6 & 16 \\ 0 & 20 \\ 18 & 28 \end{bmatrix} 
```
<br>


> **Definition (Linear/Tuple Index Conversion for Reverse Ordering)**  
> The **strides** for the **reverse ordering** are $s_d^{\*} = 1$ and $s_{k-1}^{\*} = \prod_{l=k}^dn_l = s_k^{\*}n_k$ for $k=d,\dots ,2$.  
> The **(reverse) linear index** of $(i_1, i_2, \dots , i_d) \in [n_1] \otimes [n_2] \otimes \dots \otimes [n_d]$ is $\alpha = \mathbb{L}^{\*}(i_1,i_2, \dots , i_d) = 1 + \sum_{k=1}^ds_k^{\*}(i_k - 1)$.  
> The **(reverse) tuple index** of $\alpha \in [N]$ with $N = \prod_{k=1}^dn_k$ is $(i_1,i_2, \dots ,i_d) = \mathbb{T}^{\*}(\alpha)$, where $i_k = \mathbb{T}_k^{\*}(\alpha) = 1 + \left\lfloor\dfrac{(\alpha - 1) \bmod (n_ks_k^{\*})}{s_k^{\*}}\right\rfloor$.
> 
...
## 2.1.3 General Ordering

As always, the linear index using any mode ordering $\pi = (\pi_1 ,\pi_2, \pi_3)$ by using the strides,
```math
\bar{s}_{\pi_1} = 1 \text{, } \bar{s}_{\pi_2} = \begin{cases} m \text{ if } \pi_1 = 1 \\
                                                              n \text{ if } \pi_1 = 2 \\
                                                              p \text{ if } \pi_1 =3 \end{cases},
\bar{s}_{pi_3} = \begin{cases} mn \text{ if } (\pi_1, \pi_2) = (1,2) \text{ or } (2,1) \\
                               mp \text{ if } (\pi_1, \pi_2) = (1,3) \text{ or } (3,1) \\
                               np \text{ if } (\pi_1, \pi_2) = (2,3) \text{ or } (3,2) \end{cases}.
```

 Using those strides, the linear index $l$ corresponding of $(i,j,k)$ can be computed as 

 ```math
\mathbb{L}^{(\pi)}(i,j,k) = 1 + \bar{s}_1(i - 1) + \bar{s}_2(j - 1) + \bar{s}_3(k - 1),
```

and $\pi$ is all the permutations of the indices. And like before, the tuple can be computed using the inverse function $(i,j,k) = \mathbb{T}^{\pi}(l)$, where

```math
\begin{aligned}
i = \mathbb{T}_1^{(\pi)}(l) = 1 + \left\lfloor\dfrac{(l - 1) \bmod m\bar{s}_1}{\bar{s}_1}\right\rfloor
\\[8pt]
j =  \mathbb{T}_2^{(\pi)}(l) = 1 + \left\lfloor\dfrac{(l - 1) \bmod n\bar{s}_2}{\bar{s}_2}\right\rfloor
\\[8pt]
k =  \mathbb{T}_3^{(\pi)}(l) = 1 + \left\lfloor\dfrac{(l - 1) \bmod p\bar{s}_3}{\bar{s}_3}\right\rfloor
\end{aligned}
```



> **Definition (Linear/Tuple Index Conversion for General Ordering)**  
> The **strides** for the **general ordering** are  
> $\bar{s}_{\pi_1} = 1$ and  
> $ \bar{s}_{\pi_{k+1}} = \prod_{l=1}^k n_{\pi_l} = \bar{s}_{\pi_k} n_{\pi_k} $ for $ k \in [d - 1] $.  
>
> The **(general) linear index** of  
> $ (i_1, i_2, \dots , i_d) \in [n_1] \otimes [n_2] \otimes \dots \otimes [n_d] $ is  
> $ \alpha = \mathbb{L}^{(\pi)}(i_1, i_2, \dots , i_d)
>   = 1 + \sum_{k=1}^d \bar{s}_k (i_k - 1) $.  
>
> The **(general) tuple index** of $ \alpha \in [N] $ with  
> $ N = \prod_{k=1}^d n_k $ is  
> $ (i_1, i_2, \dots , i_d) = \mathbb{T}^{(\pi)}(\alpha) $, where  
> $ i_k = \mathbb{T}_k^{(\pi)}(\alpha)
>   = 1 + \left\lfloor
>       \dfrac{(\alpha - 1) \bmod (n_k \bar{s}_k)}{\bar{s}_k}
>     \right\rfloor $.  

This f****** Latex thing doesn't want to render... I wrote it down in math block.

**Definition (Linear/Tuple Index Conversion for General Ordering)**  
The **strides** for the **general ordering** are  
```math
\bar{s}_{\pi_1} = 1 \text{ and }  \bar{s}_{\pi_{k+1}} = \prod_{l=1}^k n_{\pi_l} = \bar{s}_{\pi_k} n_{\pi_k}  for  k \in [d - 1] 
```
The **(general) linear index** of  
```math
(i_1, i_2, \dots , i_d) \in [n_1] \otimes [n_2] \otimes \dots \otimes [n_d]  \text{ is }  
 \alpha = \mathbb{L}^{(\pi)}(i_1, i_2, \dots , i_d)
  = 1 + \sum_{k=1}^d \bar{s}_k (i_k - 1)
```
The **(general) tuple index** of 
```math
\alpha \in [N] \text{ with }  
N = \prod_{k=1}^d n_k  \text{ is }  
```
```math
 (i_1, i_2, \dots , i_d) = \mathbb{T}^{(\pi)}(\alpha), \text{ where }  
 i_k = \mathbb{T}_k^{(\pi)}(\alpha)
   = 1 + \left\lfloor
       \dfrac{(\alpha - 1) \bmod (n_k \bar{s}_k)}{\bar{s}_k}
     \right\rfloor.
...
To put it in a nutshell, linear indexing assigns a unique scalar index to each tensor entry by combining its mode indices with strides, so that we can move seamlessly between multi-indices $(i_1, \dots,i_d)$ and a single index $\alpha$ while preserving a chosen memory ordering.
...
### Solutions - Exercise 2.9

<br><br>

> **Exercise 2.10** <br>

### Solutions - Exercise 2.10

<br><br>

> **Exercise 2.11** (Reverse Vector Kronecker Product)<br>
> Let $a \in \mathbb{R}^m$, $b \in \mathbb{R}^n$ and $c \in \mathbb{R}^p$, and define $v = c \otimes b \otimes a \in \mathbb{R}^{mnp}$. <br>
> Show $v_l = a_ib_jc_k$, where $l = \mathbb{L}(i,j,k)$ or $(i,j,k) = \mathbb{T}(l)$.

### Solution - Exercise 2.11

(I'm not sure at all)  
So let's take the Kronecker product of $c \otimes b \otimes a$, $a \in \mathbb{R}^m$, $b \in \mathbb{R}^n$ and $c \in \mathbb{R}^p$, with the indices are:
```math
v = c \otimes b \otimes a = \begin{bmatrix} c_1b_1a_1 \\
                                        c_1b_1a_2 \\
                                        \dots \\
                                        c_1b_1a_m \\
                                        c_1b_2a_1 \\
...
\end{bmatrix} 
```
Therefore, each entry of $v$ is of the form $v_l = a_ib_jc_k$, where the linear index $l = \mathbb{L}(i,j,k)$, or equivalently $(i,j,k) = \mathbb{T}(l)$.

<br> <br>

> **Exercise 2.12** (Reverse Khatri-Rao Product)
> Let $\mathbb{A} \in \mathbb{R}^{m \times r}$, $\mathbb{B} \in \mathbb{R}^{n \times r}$, $\mathbb{C} \in \mathbb{R}^{p \times r}$, and define their *reverse* Khatri-Rao product $V = C \odot B \odot A.$  
> Prove $v_{\alpha l} = a_{il}b_{jl}c_{kl}$ for all $(i,j,k,l) \in [m] \otimes [n] \otimes [p] \otimes [r]$ and $\alpha = \mathbb{L}(i,j,k)$.

<br> <br>

> **Exercise 2.13** (Reverse Kronecker Product)  
> Let $\mathbb{A} \in \mathbb{R}^{m \times q}$, $\mathbb{B} \in \mathbb{R}^{n \times r}$, $\mathbb{C} \in \mathbb{R}^{p \times s}$, and define their *reverse* Kronecker Product $V = C \otimes B \otimes A$.  
> a) What is the size of $V$?  
> b) What is the mapping such that $v_{\alpha \beta} = a_{i_1j_1}b_{i_2j_2}c_{i_3j_3}$?

### Solutions - Exercise 2.13

```math
V = C \otimes B \otimes A \in \mathbb{R}^{(m n p)\times(q r s)},
\qquad
v_{\alpha\beta} = a_{i_1 j_1} b_{i_2 j_2} c_{i_3 j_3},



\alpha = i_1 + (i_2-1)m + (i_3-1)mn,
\qquad
\beta = j_1 + (j_2-1)q + (j_3-1)qr.
```
...
> **Exercise 2.14**  
> Extend the functions *lin2tuple* and *tuple2lin* from Exercise 2.6 with an option to support reverse ordering.

### Solution - Exercise 2.14

```python
function lin2tup(alpha, n[1..d], order):
    if order == "natural":
        s[1] = 1
        for k = 1 to d-1:
            s[k+1] = s[k] * n[k]
...
function tup2lin(i[1..d], n[1..d], order):
    if order == "natural":
        s[1] = 1
        for k = 1 to d-1:
            s[k+1] = s[k] * n[k]
...
> **Exercise 2.16** <br>
> What is $\pi$ for the natural ordering? <br>
> For the reverse ordering?

### Solutions - Exercise 2.16
$\pi$ for the natural ordering is the identity permutation $(1,2,\dots,d)$, and for the reverse ordering $(d,d-1,\dots,1)$.

<br> <br>

> **Exercise 2.17** <br>
> Consider the domain $[m] \otimes [n] \otimes [p]$ and the permuted ordering $\pi = (2,1,3)$. <br>
> a) What is the formula for the linear index $l$ of $(i,j,k)$? <br>
> b) Given the linear index $l$, what is $(i,j,k)$?

### Solutions - Exercise 2.17

The formula is $\mathbb{L}^{(\pi)}(2,1,3) = 1 + (j - 1) + n(j - 1) + mn(k - 1)$. <br>
And with the linear index, we have $(2,1,3)$ so $(j,i,k)$:
```math
\begin{aligned}
i = 1 + \left\lfloor\dfrac{(l - 1) \bmod mn}{n}\right\rfloor
\\[8pt]
j = 1 + \left\lfloor(l - 1) \bmod n\right\rfloor
\\[8pt]
k = 1 + \left\lfloor\dfrac{(l - 1)}{nm}\right\rfloor
\end{aligned}
```
...
> **Exercise 2.18** <br>
> How many different mode orderings are possible for a *d*-tuple?

### Solution - Exercise 2.18

There are $d!$ permutations possible.
