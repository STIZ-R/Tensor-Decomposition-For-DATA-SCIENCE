...
> Let $x$ be a tensor of size $n_1 \times n_2 \times \dots \times n_d$ and $\mathbf{x} = vec(x)$. Let $\pi = (\pi_1, \pi_2, \dots, \pi_d)$ and $\mathbf{y} = \mathbb{P}(x, \pi).$ Define $\mathfrak{t} = [N] \arrow [N]$ by <br>
> $\mathfrak{t} \equiv 1 + \sum_{k=1}^d \lfloor\left\frac{(\alpha - 1) \bmod n_k\bar{s_k}{\bar{s_k}\rfloor s_k\right
