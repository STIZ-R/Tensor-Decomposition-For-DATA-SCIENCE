...
> When $\pi = (k, 1, \dots, k-1, k+1, \dots, d).$ corresponding to a mode-*k* unfolding, we use the notation $\mathbf{P}_k$ such that <br>
> $\mathbf{P_{k}} vec(\mathbf{x}) = vec(\mathbb{P}(\mathbf{X_{(k)}})$.
